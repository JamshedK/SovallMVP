...
const ForgotPassword = () => {
	...
	const onContinueButtonClicked = (e) => {
		...
		.then(a=>{alert(`Password reset email sent to ${email}. You may need to check your spam folder`)});
	}

	return (
		<div className="relative bg-green-5 h-full w-full sm:text-white flex justify-center pt-20">
			<div className=" w-[20rem]  xl:w-[25rem] text-white flex flex-col items-center gap-8">
				...
				<form onSubmit={onContinueButtonClicked} className="w-full flex flex-col items-center gap-6">
					...
					<button 
						className="block bg-white text-green-2  dropshadow-xl rounded-full px-4 py-1 mt-2 font-medium disabled:bg-gray-200 disabled:hover:cursor-no-drop" 
						disabled={!canSubmit}>Continue</button>
				</form>
			</div>
